import {
  ...
  CrudFilters,
  HttpError,
  useTranslate, useDelete, usePermissions
...
> = ({ initialData }) => {
  ...
  return (
    <AccessWrapperForPage resource={EAppResources.MARKETS} action={EPageActions.LIST}>
      <Row gutter={[16, 16]}>
            ...
            <Table rowKey="id" {...tableProps}>
              ...
              <Table.Column<IMarket>
                ...
                render={(value, record) => <div className={"preview-entity-title"}>
                  ...
                  <Link to={`/markets/show/${record.id}`} style={{marginLeft: record?.imageRaster ? '12px' : 0}}>{value}</Link>
                </div>}
              />
              ...
              <Table.Column
                ...
                key="location"
                title="Координаты"
                render={(value) => value?.coordinates && (
                    ...
                    overlay={<TableMenu record={record}
                                        ...
                                        linkButton={(
                                          <Menu.Item key="link" icon={<Icons.LinkOutlined className={"menu-item-icon"}/>}
                                            ...
                                            <Link to={`/outlets?market_id=${record.id}`}>Перейти к точкам продаж</Link>
                                          </Menu.Item>
                                        )}
                                        deleteItem={() => {
                                          mutateDelete({
